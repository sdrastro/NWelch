...
# Check for valid model parameter guesses
def check_param_guesses(guesses):
    error_message = 'Parameter guesses not understood. Must be numpy array' \
                    +'\n or list of length 2. Setting defaults.'
    if not (isinstance(guesses, np.ndarray) or isinstance(guesses, list)):
        print(error_message)
        return False
    elif (len(guesses) != 2) or (type(guesses[0]) != float) \
                  or type(guesses[1] != float)):
        print(error_message)
        return False
    else:
        return True
